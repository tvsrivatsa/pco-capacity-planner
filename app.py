...
<div class="insight-box">
    Use this table as the baseline for weekly shift planning. The "Recommended" column targets
    <strong>{target_util:.0f}%</strong> utilization on average demand for <strong>{track}</strong> track.
</div>
<div class="chart-card" style="margin-top:16px; overflow-x:auto;">
    <table>
        <thead><tr><th>Day</th><th>Shift</th><th>Current</th><th>Recommended</th><th>Gap</th><th>Forecast</th><th>Util%</th></tr></thead>
...
def main():
    ...
    st.markdown('<div class="sub-header">Upload SPC export to generate demand forecast and capacity plan</div>', unsafe_allow_html=True)
    ...
    if uploaded_spc is not None:
        ...
        if st.session_state.get('processed', False):
            ...
            st.markdown(f"**Period:** {data['Date'].min().strftime('%b %Y')} - {data['Date'].max().strftime('%b %Y')} | "
                       ...
                       f"**Target Utilization:** {target*100:.0f}%")
