...
- Adaptive utilization targets per track (Basis=60%, SM=65%, DB=70%)
...
# Adaptive utilization targets per track
TARGET_UTIL = {
    'Basis': 0.60,
    'SM': 0.65,
    'DB': 0.70,
}
DEFAULT_TARGET_UTIL = 0.65
...
def compute_capacity_plan(forecaster, start_date, track='Basis', days=90,
    ...
    target_util = TARGET_UTIL.get(track, DEFAULT_TARGET_UTIL)
    plan = []
...
def generate_html_dashboard(data, plan_df, forecaster, team_id, team_name,
    ...
    target_util = TARGET_UTIL.get(track, DEFAULT_TARGET_UTIL) * 100
...
<div class="kpi-grid">
    ...
    <div class="kpi-card"><div class="label">Volume Trend</div><div class="value">{volume_trend:+.1f}%/wk</div></div>
    <div class="kpi-card"><div class="label">SR Overhead</div><div class="value">{sr_overhead_weekly:.0f} hrs/wk</div></div>
</div>
...
<div class="footer">
    PCO TechOps Shift Capacity Planning Tool v4 | Adaptive Targets: Basis=60%, SM=65%, DB=70% | MMI SDs excluded from capacity consumption
</div>
...
def main():
    ...
    st.markdown('<div class="sub-header">Upload SPC export + incoming volumes for enhanced forecasting with effort-based workload analysis</div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown("**Adaptive Targets by Track:**")
        st.markdown("""
        - Basis: **60%** utilization
        - SM: **65%** utilization
        - DB: **70%** utilization
        """)
        ...
        st.markdown("---")
        st.markdown("**v4** | Adaptive Targets | MMI-Aware")
    ...
    if uploaded_spc is not None:
        ...
        target = TARGET_UTIL.get(track, DEFAULT_TARGET_UTIL)
        ...
        if st.session_state.get('processed', False):
            ...
            # KPI Metrics Row 2: New metrics
            col6, col7, col8, col9 = st.columns(4)
            col6.metric("Target Util", f"{target*100:.0f}%", delta=f"{track} track")
            col7.metric("Volume Trend", f"{volume_trend:+.2f}%/wk",
                       delta="growing" if volume_trend > 0 else "stable" if volume_trend == 0 else "declining")
            col8.metric("SR Overhead", f"{sr_overhead:.1f} hrs/shift",
                       delta=f"{sr_overhead_weekly:.0f} hrs/wk total")
            col9.metric("Trend Factor", f"{forecaster.trend_factor:.3f}")
            ...
            # Volume Trend Chart (if volume data provided)
            if volume_trend != 0.0 and uploaded_volumes is not None:
                st.markdown("#### Expected Volume Trend")
                try:
                    vol_xls = pd.ExcelFile(uploaded_volumes)
                    target_sheet = find_partner_sheet(vol_xls.sheet_names, partner_name)
                    if target_sheet:
                        vol_data = pd.read_excel(vol_xls, sheet_name=target_sheet)
                        cw_cols = [c for c in vol_data.columns if c.startswith('CW')]
                        csr_row = vol_data[vol_data['Request Type'] == 'Customer Service Request']
                        sr_row = vol_data[vol_data['Request Type'] == 'Service Request']
                        
                        if len(csr_row) > 0:
                            fig_vol = go.Figure()
                            csr_vals = csr_row[cw_cols].values.flatten()
                            fig_vol.add_trace(go.Scatter(
                                name='CSR Volume', x=cw_cols, y=csr_vals,
                                mode='lines+markers', line=dict(color='#0070F2', width=2)))
                            
                            if len(sr_row) > 0:
                                sr_vals = sr_row[cw_cols].values.flatten()
                                fig_vol.add_trace(go.Scatter(
                                    name='SR Volume (Internal)', x=cw_cols, y=sr_vals,
                                    mode='lines+markers', line=dict(color='#E76500', width=2, dash='dash')))
                            
                            # Add trend line
                            steady_cols = [c for c in cw_cols if c != 'CW01']
                            steady_vals = csr_row[steady_cols].values.flatten().astype(float)
                            x_trend = np.arange(len(steady_vals))
                            slope, intercept = np.polyfit(x_trend, steady_vals, 1)
                            trend_line = slope * x_trend + intercept
                            fig_vol.add_trace(go.Scatter(
                                name=f'Trend ({volume_trend:+.2f}%/wk)', x=steady_cols, y=trend_line,
                                mode='lines', line=dict(color='#36A41D', width=2, dash='dot')))
                            
                            fig_vol.update_layout(
                                title=f'Weekly Incoming Volume - {partner_name}',
                                yaxis_title='Requests', height=300,
                            ...
                            st.markdown(f"""
                            <div class="info-box">
                                <strong>Volume Trend:</strong> {volume_trend:+.2f}% per week (linear trend on CW02-CW15). 
                                SR Overhead: {sr_overhead_weekly:.0f} hrs/week of coordination effort for {sr_row[cw_cols[1:]].mean(axis=1).values[0]:.0f} internal SRs/week.
                            </div>
                            """, unsafe_allow_html=True)
                except Exception:
                    pass
            ...
            with param_col2:
                ...
                | SR Overhead (weekly) | {sr_overhead_weekly:.0f} hrs/week |
